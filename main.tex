...
compression rate \citet{duda2009}. The key difference between ANS and AC is
that ANS is last-in-first-out (LIFO), or `stack-like', while AC is
first-in-first-out (FIFO), or `queue-like'.
...
\begin{equation}
  \push:(m, x) \mapsto m'.
\end{equation}
The new compressed state, \(m'\), contains precisely the same information as
the pair \((m, x)\), and therefore \push\ can be inverted, giving a decoder
...
An algorithm which solves \Cref{prob:alt} has an inverse which solves
\Cref{prob:default}, and vice versa. Our description of the details of ANS
focuses on the decoding algorithm, because this leads to a more straightforward
presentation. We describe the decoder and show that it solves \Cref{prob:alt},
