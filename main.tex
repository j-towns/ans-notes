...
\begin{align}
      &&2^{r_s - r_t}            \leq 2^{r} \cdot (s' \div p) + s' \bmod p + c &< 2^{r_s}\label{eq:o-simple-cond}\\
  \Rightarrow&& 2^{r} \cdot (s' \div p) &< 2^{r_s} - s' \bmod p - c \\
  \Rightarrow&& s' \div p &< 2^{r_s - r} - (s' \bmod p + c)2^{-r}\label{eq:simple-cond}
\end{align}
So \texttt{renorm\_inverse} should move data from the lower order bits of
\(s'\) into \(t\) (decreasing \(s'\)) until \cref{eq:simple-cond}, which is equivalent to the right inequality of \cref{eq:o-simple-cond}, is satisfied.
To be specific:

\begin{lstlisting}[frame=single]
def renorm_inverse($s'$, $t$; $p$, $c$):
    while $s' \div p \geq 2^{r_s - r} - (s' \bmod p + c)2^{-r}$:
        $t$ := stack_push($t$, $s'\bmod 2^{r_t}$)
        $s'$ := $s'\div 2^{r_t}$
    return $s'$, $t$
\end{lstlisting}

Although, as mentioned above, \cref{eq:o-simple-cond} is a \emph{necessary}
...
land in the interval specified by \cref{eq:o-simple-cond} at most once. This
